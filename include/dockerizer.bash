...
readonly app_path="${APP_PATH:-/tmp/app}"
readonly env_path="${ENV_PATH:-/tmp/env}"
readonly build_path="${BUILD_PATH:-/tmp/build}"
readonly cache_path="${CACHE_PATH:-/tmp/cache}"
readonly import_path="${IMPORT_PATH:-/app}"
readonly dockerfile_path="${DOCKERFILE_PATH:-/tmp/dockerfiles}"
...
paths() {
  ...
  printf "%-32s # %s\n" \
    ...
    "DOCKERFILE_PATH=$dockerfile_path" "Path to dockerfile templates" \
    "BUILDPACK_PATH=$buildpack_path" "Path to installed buildpacks"
}

version() {
  declare desc="Show version and supported version info"
  echo "dockerizer: ${DOCKERISER_VERSION:-dev}"
  echo "buildpacks:"
  asset-cat include/buildpacks/lastbackend.txt | sed 's/.*lastbackend\///' | xargs printf "  %-26s %s\n"
  asset-cat include/buildpacks/heroku.txt      | sed 's/.*heroku\///'      | xargs printf "  %-26s %s\n"
  asset-cat include/buildpacks/custom.txt      | xargs printf "  %-26s %s\n"
}
...
dockerizer-test() {
  declare desc="Test running an app through dockerizer"
...
main() {
  ...
  cmd-export paths
  cmd-export version
  cmd-export dockerizer-test test

  cmd-export-ns buildpack "Use and install buildpacks"
  cmd-export buildpack-sources
  cmd-export buildpack-detect
  cmd-export buildpack-build
  cmd-export buildpack-export
  cmd-export buildpack-cleanup
  ...
  case "$SELF" in
    ...
    /sources)    buildpack-sources "$@";;
    /detect)     buildpack-detect "$@";;
    /build)      buildpack-build  "$@";;
    /export)     buildpack-export "$@";;
    /cleanup)    buildpack-cleanup "$@";;
    *)      cmd-ns "" "$@";
  esac
}
